...
def _verify_arg(verify_ssl):
	# Match the existing library behavior (hostname verification disabled).
	return False


def validate_certificate_files(ca_cert, client_cert, client_key):
	if not os.path.exists(ca_cert):
		return "CA certificate not found: {0}".format(ca_cert)
	if not os.path.exists(client_cert):
		return "Client certificate not found: {0}".format(client_cert)
	if not os.path.exists(client_key):
		return "Client key not found: {0}".format(client_key)
	return None


def check_opensearch(host, port, timeout, ca_cert, client_cert, client_key, verify_ssl):
	cert_error = validate_certificate_files(ca_cert, client_cert, client_key)
	if cert_error:
	...
	try:
		response = requests.get(
			...
			verify=_verify_arg(verify_ssl),
			timeout=timeout
		)
		if response.status_code == 200:
			return True, None
		return False, "OpenSearch returned HTTP status {0}".format(response.status_code)
	except requests.exceptions.RequestException as exc:
		return False, str(exc)


def index_exists(host, port, timeout, index_name, ca_cert, client_cert, client_key, verify_ssl):
	...
	try:
		...
		if response.status_code == 404:
			return False, None
		return None, "Failed checking index '{0}': HTTP {1}".format(index_name, response.status_code)
	except requests.exceptions.RequestException as exc:
		return None, str(exc)


def delete_index_if_exists(host, port, timeout, index_name, ca_cert, client_cert, client_key, verify_ssl):
	index_url = "{0}/{1}".format(_build_base_url(host, port), index_name)
	try:
		...
		if head_response.status_code != 200:
			return None, "Failed checking index '{0}' before purge: HTTP {1}".format(index_name, head_response.status_code)
		...
		if delete_response.status_code in (200, 202):
			return True, None
		return None, "Failed deleting index '{0}': HTTP {1} body={2}".format(
			index_name, delete_response.status_code, delete_response.text
		)
	except requests.exceptions.RequestException as exc:
		return None, str(exc)
...
def get_shard_capacity(host, port, timeout, ca_cert, client_cert, client_key, verify_ssl):
...
def attach_ism_policy_to_index(host, port, timeout, index_name, ca_cert, client_cert, client_key, verify_ssl, policy_id=_DEFAULT_KEEP_FOREVER_POLICY_ID):
...
def ensure_index_template(
	...
	client_key,
	verify_ssl,
	policy_id=_DEFAULT_KEEP_FOREVER_POLICY_ID,
	created_timestamp_field="report_created_at",
	updated_timestamp_field="report_updated_at",
	synced_timestamp_field="report_synced_at"
):
...
def get_index_mapping(host, port, timeout, index_name, ca_cert, client_cert, client_key, verify_ssl):
...
def ensure_geo_point_field_mapping(host, port, timeout, index_name, field_name, ca_cert, client_cert, client_key, verify_ssl):
...
def set_cluster_max_shards_per_node(host, port, timeout, new_limit, ca_cert, client_cert, client_key, verify_ssl):
...
def refresh_index(host, port, timeout, index_name, ca_cert, client_cert, client_key, verify_ssl):
...
def run_module():
	...
	is_ok, conn_err = check_opensearch(es_host, es_port, es_timeout, ca_cert, client_cert, client_key, verify_ssl)
	if not is_ok:
		module.fail_json(msg="OpenSearch is not reachable. {0}".format(conn_err), **result)

	already_exists, exists_err = index_exists(es_host, es_port, es_timeout, target_index, ca_cert, client_cert, client_key, verify_ssl)
	if exists_err:
		module.fail_json(msg=exists_err, **result)
	...
	if purge_before_load:
		index_purged, purge_err = delete_index_if_exists(
			es_host,
			es_port,
			es_timeout,
			target_index,
		...
		if purge_err:
			module.fail_json(msg=purge_err, **result)
